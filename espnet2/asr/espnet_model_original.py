...
class ESPnetASRModel(AbsESPnetModel):
        ...
        **kwargs,
        ...
        if self.use_transducer_decoder:
            ...
            if self.ctc_weight == 0.0:
                ...
                #lang_weight = 0.3
                loss = self.ctc_weight * (moe_loss_ctc) + (1 - self.ctc_weight) * loss_att + man_loss_ctc + eng_loss_ctc
    ...
    def encode(
        ...
        if self.encoder.interctc_use_conditioning:
            ...
            man_encoder_out, eng_encoder_out, shared_encoder_out, encoder_out_lens, _ = self.encoder(feats, feats_lengths)
        intermediate_outs = None
        if isinstance(man_encoder_out, tuple):
            intermediate_outs = man_encoder_out[1]
            man_encoder_out = man_encoder_out[0]
        ...
        if (
            ...
            assert moe_encoder_out.size(1) <= encoder_out_lens.max(), (
                moe_encoder_out.size(),
                encoder_out_lens.max(),
            )
        ...
        return man_encoder_out, eng_encoder_out, moe_encoder_out, encoder_out_lens, man_weight, eng_weight
