...
class Speech2Text:
    ...
    def __init__(
        ...
        ctc = CTCPrefixScorer(ctc=asr_model.ctc_moe, eos=asr_model.eos)
        ctc_inter = CTCPrefixScorer(ctc=asr_model.ctc_inter, eos=asr_model.eos)
    ...
    def __call__(
        ...
        # b. Forward Encoder
        man, eng, enc, intermediate_outs, _ = self.asr_model.encode(**batch)
        # # logging.info(f"man--{man.shape}")
        # # logging.info(f"eng--{eng.shape}")
        # torch.set_printoptions(threshold=np.inf)
        # with open("embedding",'a')as f: 
        #     data_man={'man':man.numpy().tolist()}
        #     json.dump(data_man,f)
        #     f.write("\n")
        #     data_eng={'eng':eng.numpy().tolist()}
        #     json.dump(data_eng,f)
        #     f.write("\n")
        #     data_enc={'enc':enc.numpy().tolist()}
        #     json.dump(data_enc,f)
        #     f.write("\n")
        #     w_man={'w_man':man_weight.numpy().tolist()}
        #     json.dump(w_man,f)
        #     f.write("\n")
        #     w_eng={'w_eng':eng_weight.numpy().tolist()}
        #     json.dump(w_eng,f)
        #     f.write("\n")
        if self.multi_asr:
    ...
    def _decode_interctc(
        ...
        for layer_idx, encoder_out in intermediate_outs:
            y = self.asr_model.ctc_inter.argmax(encoder_out)[0]  # batch_size = 1
            logging.info(f"y-----------------{y}")
